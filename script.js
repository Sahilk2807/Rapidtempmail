// --- FIX FOR INFINITYFREE ?i=1 ---
// This code must be at the very top to run before anything else.
(function() {
...
document.addEventListener('DOMContentLoaded', () => {
    // --- STATE MANAGEMENT ---
    let currentAccount = null; // Will store { address, id, token }
    ...
    const fetchInboxMessages = async () => {
        ...
        try {
            ...
            if (response.status === 401) {
                console.error('Token expired or invalid. Stopping refresh.');
    ...
    const clearInbox = () => {
        ...
        inboxPlaceholder.innerHTML = '<p>Generate an email to see your messages.</p>';
        inboxPlaceholder.style.display = 'block';
    };
    ...
    const setInboxStatus = (status) => {
        switch(status) {
            case 'fetching': inboxStatus.innerHTML = '📨'; inboxStatus.classList.add('animate-bounce'); break;
            case 'error': inboxStatus.innerHTML = '⚠️'; inboxStatus.classList.remove('animate-bounce'); break;
            default: inboxStatus.innerHTML = ''; inboxStatus.classList.remove('animate-bounce'); break;
        }
    };

    const createMessageElement = (message) => {
        const item = document.createElement('div');
        item.className = 'message-item p-3 bg-gray-700 rounded-md hover:bg-gray-600 cursor-pointer transition-colors duration-200';
        item.dataset.id = message.id;

        item.innerHTML = `
            <div class="flex justify-between items-start">
                <p class="font-bold text-white truncate pr-4">${message.from.address}</p>
                <p class="text-xs text-gray-400 flex-shrink-0">${new Date(message.createdAt).toLocaleTimeString()}</p>
            </div>
            <p class="text-gray-300 truncate">${message.subject}</p>
        `;
        return item;
    };
    ...
    window.addEventListener('keydown', (e) => {
        if (e.key === 'Escape' && !messageModal.classList.contains('hidden')) closeModal();
    });
    
    inboxMessages.addEventListener('click', (e) => {
        const messageItem = e.target.closest('.message-item');
        if (messageItem) {
            fetchAndShowMessage(messageItem.dataset.id);
        }
    });
